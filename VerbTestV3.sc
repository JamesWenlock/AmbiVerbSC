...
.stringColor_(Color.green).align_(\center);
			data.do({arg thisData, i;
				...
				Knob(outView, 30@25).color_([Color.black, Color.green, Color.green, Color.green])
				...
				StaticText(outView, 105@30).string_(thisData[0].asString)
	...
	createParams.value(
		...
		createParams.value(
			...
			[
				...
				[\coupAmt, 2pi, [0, 9pi], 10, True],
				[\rotRate,0.221, [0, 10], 11, True],
				[\rotAmt,2pi, [0, 9pi], 12, True],
				[\rotMix,1, [0, 1], 13, True],
			],
